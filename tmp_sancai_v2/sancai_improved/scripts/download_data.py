...
DEPRECATED: prefer scripts/fill_gaps.py for gap-filling and data updates.
This script is kept for bulk initial downloads only.
"""
import argparse
import json
import logging
...
from scripts.data_schema import ADJUST_MODE, write_daily_parquet, write_minute_parquet

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger(__name__)

logger.warning("DEPRECATED: scripts/download_data.py is deprecated. Use scripts/fill_gaps.py for data updates.")
...
def download_daily(symbol: str, name: str, conn: sqlite3.Connection,
    ...
    try:
        ...
        df = ak.stock_zh_a_hist(
            ...
            end_date=end_date,
            adjust=ADJUST_MODE  # forward-adjusted -> now consistent with data_schema
        )
        ...
        if file_path.exists():
            try:
                ...
                logger.warning(f"Corrupt parquet for {symbol}, re-downloading: {e}")
        ...
        write_daily_parquet(df, file_path)
...
def download_minute(symbol: str, name: str, conn: sqlite3.Connection,
    ...
    try:
        ...
        write_minute_parquet(df, file_path)
